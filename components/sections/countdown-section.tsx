...
export function CountdownSection() {
  ...
  return (
    <MotionSection className="bg-[linear-gradient(180deg,#07050d,#10091b)]">
      <AmbientBackground />
      <div className="section-shell relative">
        <motion.div variants={motionItemVariants} className="mx-auto max-w-3xl text-center">
          ...
          <h2 className="mt-4 font-serif text-5xl leading-tight text-white md:text-7xl">O güne kalan zarif heyecan</h2>
        </motion.div>
        <div className="mt-14 grid grid-cols-2 gap-4 md:grid-cols-4 md:gap-6">
          {units.map(([label, value]) => (
            <motion.div
              ...
              key={label}
              className="glass dark-panel purple-glow rounded-2xl p-6 text-center text-white md:p-8"
            >
              ...
              <span className="mt-3 block text-xs font-semibold uppercase tracking-[0.28em] text-white/46">{label}</span>
            </motion.div>
          ))}
        </div>
      </div>
